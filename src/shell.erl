...
-export([report_exception/4]).
-export([pp/3,enc/0,severity_tag/1,encoding/0]).
...
report_exception(Class, Severity, {Reason,Stacktrace}, RT) ->
    %% typically Class=error, Severity=serious Reason = {badmatch,4}
    %% Tacktrace = [{erl-eval,expr,3,[]}]
    Tag = severity_tag(Severity),
    %% Tag = <<*>> <**>> <<"**">>
    I = iolist_size(Tag) + 1,  %% 4 or something
